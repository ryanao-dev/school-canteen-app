# School Canteen App

Instructions to open the School Canteen App Code in VS Code.

Open the prepared code from the following GitHub repo, URL:

<https://github.com/ryanao-dev/school-canteen-app>

---

1. Click the *Code* button.

![](docs/images/image1.png)

---

2. Click *Download ZIP.*

![](docs/images/image2.png)

---

3. Here you see the downloaded zip in file explorer Downloads.

![](docs/images/image3.png)

---

4. Right click on school-canteen-app-main zip and click *Extract All*.

![](docs/images/image4.png)

---

5. Click *Extract*.

![](docs/images/image5.png)

---

6. Open Visual Studio Code from the desktop.

![](docs/images/image6.png)

---

7. From the Start Screen click *Open Folder* (or go to File → Open Folder)

![](docs/images/image7.png)

---

8. Navigate to Downloads and select school-canteen-app-main folder and click *Select Folder*.

![](docs/images/image8.png)

---

9. Click *Yes, I trust the authors*.

![](docs/images/image9.png)

---

10. Open App in web browser

- Back from file explorer

- Double click on start Windows Batch File (not SH Source file) in Downloads school-canteen-app-main folder.

![](docs/images/image10.png)

- Click on *Run*.

- Ignore any of the warnings

- This will take a few minutes to fully load

![](docs/images/image11.png)

- Black command windows will open which are ok.

- The App should then open in the default web browser.

![](docs/images/image12.png)

- If the School Canteen app doesn't open in the web browser hold down the ctrl button and click on the URL below.

![](docs/images/image13.png)

---

11. Open AI Assistant Kiro

- Click on the 3 dots at the top → Terminal → New Terminal

![](docs/images/image14.png)

- This will open a new terminal section at the bottom of the tool.

![](docs/images/image15.png)

- Type in ***kiro-cli*** and press enter (or if that fails type in ***kiro-cli --classic*** and enter)

- This will open Kiro the AI tool you will use to help with fixes and changes.

![](docs/images/image16.png)
